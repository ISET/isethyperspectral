...
landscape = rd.listArtifacts('print',true,'type','mat');
...
for ii=1:length(landscape)
    fname = landscape(ii).artifactId;
    
    chdir(sceneDir);
    fprintf('Reading %s ...',fname);
    data = rd.readArtifact(landscape(ii));
    fprintf('\n');
    imgMean = [];
    ieSaveMultiSpectralImage(fname,data.mcCOEF,data.basis,...
        data.comment,imgMean,data.illuminant,...
