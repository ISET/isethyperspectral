...
chdir(d);
if ~exist('2002','dir'),mkdir('2002'); end
...
for ii=1:length(urls)
    ...
    mType = 'canonical';
    vExplained = 0.999;
    fullFile = fullfile(pwd,'2002',[fname,'_iset.mat']);
...
for ii=3
    ...
    photons = XW2RGBFormat(reflectances*diag(ill),row,col);
    scene = sceneSet(scene,'photons',photons);
    % scene = sceneCrop(scene,r);
    % ieAddObject(scene); sceneWindow;
    ieAddObject(scene); sceneWindow;
    [scene, r] = sceneCrop(scene);
...
sz2004 = [ 1018 1339;  1017 1338; 1018 1267; 1019 1337; 1020 1339; ];
%%



%%
fname = 'ref_mosteiro4bb_reg1';

% Read the reflectances
load('ref_mosteiro4bb_reg1');
wave=400:10:710;
if size(reflectances,3) == 33
    wave=400:10:720;
end

% Make an empty scene with the
scene = sceneCreate('empty');
scene = sceneSet(scene,'wave',wave);
scene = sceneSet(scene,'illuminant',illuminantCreate('D65',wave));

% Multiply the original reflectances by the illuminant
ill = sceneGet(scene,'illuminant photons');
...
mType = 'canonical';
vExplained = 0.999;
fullFile = fullfile(pwd,[fname,'_iset.mat']);
comment = 'Original reflectances from the posted data are preserved.  Illuminant set to D65';
[~,nBases] = sceneToFile(fullFile,scene,vExplained,mType,comment);

s = sceneFromFile(fullFile,'multispectral');
ieAddObject(s); sceneWindow;


%% This is what we upload to the RDT
data.scene = scene;
